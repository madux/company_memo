...
class Memo_Model(models.Model):
    ...
    def compute_employee_supervisor(self):
        if self.employee_id:
            ...
            if current_user and self.employee_id.administrative_supervisor_id:
                ...
                self.is_supervior = False
            if current_user.id and self.employee_id.parent_id:
    ...
    def validate_final_stage(self):
        '''Docs: This disallows user from clicking the proceed button 
        when stage is in final or done stage'''
        stage_ids = self.stage_id.memo_config_id.stage_ids
        if stage_ids and stage_ids[-1].id == self.stage_id.id:
            raise ValidationError("You cannot proceed when the current stage is in Done / Final stage")  
        
    def forward_memo(self):
        self.validate_final_stage()      
    ...
    def get_initial_stage(self, memo_type, department_id):
        '''memo_type: returns Id eg 8'''
        memo_settings = self.env['memo.config'].sudo().search([
            ('memo_type', '=', memo_type),
            ('department_id', '=', department_id)
            ], limit=1)
        # raise ValidationError(f'Technically ff {memo_settings}')
    ...
    def register_state(self):
        '''update the memo state to done if the account reference is post
        Else set the state to Approve (Waiting for payment confirmation)
        if the account reference is not generated or is cancelled
        '''
        memo_account = self.env['account.payment'].search(
            [('memo_reference', '=', self.id)],
            limit=1)
        if memo_account:
            if memo_account.state not in ['posted']:
                state = 'Approve'
                self.state = state
            else:
                self.state = 'Done'
    ...
    def mail_sending_direct(self, body_msg, email_to=False): 
        ...
        approver_emails = [
            eml.work_email for eml in self.stage_id.approver_ids if eml.work_email]
    ...
    def generate_soe_entries(self):
        ...
        if is_config_approver:
            ...
            if not inv:
                ...
                # raise ValidationError(self.mapped('product_ids').filtered(lambda x: x.to_retire))
    ...
    def action_post_and_vallidate_payment(self): # Register Payment
        if self.invoice_ids:
            ...
            raise ValidationError("Error! you must provide either invoice lines or request item lines")
    ...
    def Register_Payment(self):
        if self.env.uid not in [r.user_id.id for r in self.stage_id.approver_ids]:
            raise ValidationError(
                """You are not Permitted to approve this Memo. Contact the authorized Person for payments
            """)
        ...
        computed_amount_total = sum([rec.sub_total_amount for rec in self.product_ids]) if self.product_ids else 0
        vals = {
                'name':'Memo Payment',
                'view_mode': 'form',
                'view_id': view_id.id,
        ...
        if not account_payment_existing:
            vals.update({
                'context': {
                        'default_amount': self.amountfig if self.invoice_ids else computed_amount_total if self.product_ids else 0,
                        'default_payment_type': 'outbound',
                        'default_partner_id':self.vendor_id.id or self.employee_id.user_id.partner_id.id, 
                        'default_memo_reference': self.id,
                        'default_communication': self.name,
                        'default_ref': self.code,
                },
                'domain': [],
            })
        else:
